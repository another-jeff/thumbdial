 ...
 *  - ORDINALS: `ordinals: N` declares N implicit leaves labelled "1".."N",
 *      expanded just-in-time instead of stored (a chapter's verses, a floor's
 *      rooms, a deck's cards). Keeps big trees small.
 *  - PICK MODE: what a node offers when you land on it, DECLARED by the author
 *      (never guessed from label shapes) — see `pickModeOf`.
 *
 * This module is domain-agnostic on purpose: it knows about ordinals and pick
 * modes, not about chapters, verses, or groceries.
 */
...
// A node's children. If it has none but carries an `ordinals` count, generate
// the ordinal leaves on demand ("1".."N") rather than storing them in the data.
export function childrenOf(node) {
  if (node.children && node.children.length) return node.children;
  if (node.ordinals > 0) {
    return Array.from({ length: node.ordinals }, (_, i) => ({
      label: String(i + 1),
      __ordinal: true,
    }));
  }
...
/**
 * What this node offers when it's the focused centre — the author's declaration,
 * NOT an inference from label shapes:
 *   'range' → its children are an ordinal set you sweep into ranges
 *   null    → navigate only
 * `ordinals: N` implies 'range' (that's what the field means); `pick: false`
 * opts out; `pick: 'range'` states it for authored (non-generated) children.
 * 'set' is reserved for the deferred cross-parent basket mode.
 */
export function pickModeOf(node) {
  if (!node || node.__bucket) return null;
  if (node.pick === false) return null;
  if (typeof node.pick === 'string') return node.pick;
  return node.ordinals > 0 ? 'range' : null;
}

// ---- weight helpers -------------------------------------------------------

export function leafCount(node) {
  if (node.ordinals > 0 && !(node.children && node.children.length)) return node.ordinals;
...
export function isBranch(node) {
  return !!(node.children && node.children.length) || node.ordinals > 0;
}
